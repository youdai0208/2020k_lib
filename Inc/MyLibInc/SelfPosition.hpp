...
public:
	...
	static constexpr double kControlCycle = 0;//sec
	static constexpr double kRadiansPerPulse = (2*M_PI)/ kEncoderPPR;
	static constexpr double kDistanceBetweenWheels = 0;
...
//  void calculationGlobalCoordinates(const RawEachAxisEncoder& now_encoder_value, const double machine_theta);
	...
	inline double machine_speed(void) {
		return ((kTireRadius / 2) * right_tire_angular_velocity_) + ((kTireRadius / 2) * left_tire_angular_velocity_);
	}
	inline double machine_angular_velocity(void) {
		return ((kTireRadius / kDistanceBetweenWheels) * right_tire_angular_velocity_) - ((kTireRadius / kDistanceBetweenWheels) * left_tire_angular_velocity_);
	}
	inline double machine_theta(void) {
		return machine_angular_velocity * kControlCycle;
	}
...
private:
	...
	double last_machine_theta_;
    RawEachAxisEncoder calBeforeEachAxisEncoder_;
	EachAxisEncoderWheelOdometry calAfterEachAxisEncoder_;
    AmountOfMovement amountOfMovement_;
    Coordinate globalCoordinates_;
